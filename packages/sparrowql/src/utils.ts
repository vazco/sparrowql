...
export function closestPath(relations: Relation[], starts: Relation[], ends: string[], query?: Record<string, any>) {
...
export function closestPathBFS(relations: Relation[], start: Relation, ends: string[], query?: Record<string, any>): { cost: number; path: Relation[] } {
    const visited = {[start.from]: start};
    const inQuery: string[] = Object.keys(query || {}).map(getNameCollection).filter(Boolean) as string[];
