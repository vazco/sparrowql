...
function prepare({computed, limit, projection, query, relations = [], skip, sort, start}) {
    ...
    function inlineRelation(relation) {
        ...
        steps.push({
            relation: Object.assign({}, relation, {local: relative([relation.from, relation.local].join('.'), false)})
        });
